...
p2 <- ggplot(data = alc, aes(x=prediction, col=prediction))
...
```
The mean prediction error (0.24) combined from the rounds on the testing data seems to be a little bit higher than on the training data (0.23).

The model introduced in our Exercise set 3 had about 0.25 mean prediction error on the 10-fold cross validation on the testing data which implies that our new model has a better test set performance than the one in the Exercise set (variables: sex, failures, absences).
...
We'll do a comparison of 6 different testing models, substracting the number of predictors in each model by always keeping the most significant variables (dropping the last 5 variables).
